...
(defn ^Boolean single-in? [claims ^String p]
  (= "single" (one claims p "cardinality")))
...
(defn apply-assert [claims ^Claim c]
  (if (single-in? claims (:p c)) (conj (drop-lp claims (:l c) (:p c)) c) (if (has-claim? claims c) claims (conj claims c))))

(defn apply-retract [claims ^Claim c]
  (if (single-in? claims (:p c)) (drop-lp claims (:l c) (:p c)) (filterv (fn [x] (not (claim-eq? x c))) claims)))
